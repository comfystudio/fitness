...
<?php echo $this->Html->script('google-jsapi');?>
...
<div class = 'home-right-shadow'>
    <div class = 'right-nav-content'>
        ...
        <div class = 'home-header'>
            <ul>
                <li class = 'li-active' id = "home-feed">Feed</li>
                <li id = "home-progress">Progress</li>
                <li id = "home-pictures">Pictures</li>
            </ul>
        </div>
        
        <div id="LoadingImage" style="display: none">
            <img alt = 'loading' src="../../webroot/img/ajax-loader.gif" />
        </div>
        <div class = 'home-wrapper'> 
                ...
                <div class = 'social-post'  id = 'paginator_<?php //echo ($page_no);?>'>
                	<?php
						...
						echo $form->submit('post.png', array('alt' => 'post'));
						//echo $form->end('post.png', array('alt' => 'post'));
						echo $form->end();
                    ?>
                </div>
                        ...
                        <div class = 'social-text'>
                              ...
                              <?php $count2 = 0;$count3 =  0; $test = array();
									...
									foreach ($test as $item){
											...
											if(!empty($avatar)){
												echo $html->link($html->image($urlString.$avatar['Picture']['image'], array('width' => '32', 'height' => '32', 'alt' => 'avatar')), array('controller' => 'users', 'action' => 'view',$item['user_id']), array('escape' => false, 'title' => $item['username']));
											}else{
												echo $html->link($html->image('/img/avatar.png',array('width' => '32', 'height' => '32', 'alt' => 'avatar')), array('controller' => 'users', 'action' => 'view',$item['user_id']), array('escape' => false, 'title' => $item['username']));
                                ...
                                <div class = 'comment' id = 'comment-<?php echo $post['Post']['id']?>'>
                                    <?php foreach($post['Comment'] as $comment){
										echo '<div class = "comment-each">';
											...
											if(!empty($avatar)){
												echo $html->link($html->image($urlString.$avatar['Picture']['image'], array('width' => '32', 'height' => '32',  'alt' => 'avatar')), array('controller' => 'users', 'action' => 'view',$comment['user_id']), array('escape' => false, 'title' => $username['User']['username']));
											}else{
												echo $html->link($html->image('/img/avatar.png',array('width' => '32', 'height' => '32',  'alt' => 'avatar')), array('controller' => 'users', 'action' => 'view',$comment['user_id']), array('escape' => false, 'title' => $username['User']['username']));
											}
                                    ...
                                    <div class = 'new-comment'>
										...
										<?php if(!empty($avatar)){
												...
												echo $this->Form->hidden('post_id', array('id' => 'post-id_'.$post['Post']['id'], 'value' => $post['Post']['id']));
												//echo $this->Form->hidden('user_id', array('class' => 'user_id', 'id' => 'user-id_'.$this->Session->read('User.id'), 'value' => $this->Session->read('User.id') ) );
												echo $this->Form->input('text', array('id' => 'text_'.$post['Post']['id'], 'placeholder' => 'Add comment...', 'type' =>'textarea', 'class' => 'textarea', 'rows' => '1', 'label' => false));
												echo '<div class = "error-message" id = "error-message_'.$post['Post']['id'].'"></div>';
												echo '<div class = "submit" id = "submit_'.$post['Post']['id'].'">';
													echo $this->Form->submit('submit.png', array('div'=>false, 'alt' => 'submit'));
												echo '</div>';

												//echo $this->Form->end('submit.png', array('div' => false));
										?>
                                    </div>
                                </div>
                        </div>
...
<script> feed(); commentLike(); commentAdd(); like();</script>
